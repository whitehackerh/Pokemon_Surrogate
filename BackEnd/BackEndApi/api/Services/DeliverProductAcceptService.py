...
class DeliverProductAcceptService(BaseService):
    def service(self, request):
        try:
            ...
            if record.count() == 1 and ServiceUtils.isEnableDeliverAccept(record[0].status, user_id, record[0].client_id, record[0].contractor_id):
                model.updateStatus(accept_id, AcceptStatus.DELIVERED)
            else:
